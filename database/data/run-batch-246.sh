#!/usr/bin/env bash
# Batch 246: reconcile 27 previously imported dashboard events with the repo.
# Run after git pull and the earlier pending numbered batches:
#   bash database/data/run-batch-246.sh
# Read-only preview:
#   bash database/data/run-batch-246.sh --dry-run
#
# These rows were first inserted outside the numbered-batch workflow. This
# batch records their payload and provenance; it must not insert them twice.
# Match URL AND event date: one article can document multiple dated events.
# No migrations, prisoner changes, or updates to existing dashboard records.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-246.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch246.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 246 || ($payload["expected_count"] ?? null) !== 27 || count($payload["entries"] ?? []) !== 27) {
...
run "reconcile-dashboard-events" "B246-OK" "$ADD_CODE" || exit 1
echo "Batch 246 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
