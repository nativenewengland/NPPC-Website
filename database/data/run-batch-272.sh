#!/usr/bin/env bash
# Batch 272: 4 missing FOR, AFSC and Peace Action Center activists from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-272.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch272.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 272 || ($payload["expected_count"] ?? null) !== 4 || count($payload["entries"] ?? []) !== 4) {
...
foreach ($payload["entries"] as $entry) {
    Validator::make($entry, ["key" => "required|string", "match_names" => "required|array|min:1", "match_names.*" => "required|string", "prisoner.name" => "required|string|max:255", "prisoner.first_name" => "required|string|max:255", "prisoner.last_name" => "required|string|max:255", "prisoner.description" => "required|string", "prisoner.state" => "required|string", "prisoner.era" => "required|in:1940s,1960s", "prisoner.in_custody" => "required|boolean|declined", "prisoner.released" => "required|boolean", "prisoner.lat" => "required|numeric|between:-90,90", "prisoner.lng" => "required|numeric|between:-180,180", "prisoner.cases" => "required|array|size:1", "dates" => "present|array", "case_dates" => "present|array"])->validate();
    ...
    $expected = ["alfred-hassler" => ["name" => "Alfred Hassler", "affiliation" => ["Fellowship of Reconciliation"], "state" => "Pennsylvania", "era" => "1940s", "lat" => 40.964421, "lng" => -76.884795], "lawrence-scott" => ["name" => "Lawrence Scott", "affiliation" => ["American Friends Service Committee", "Peace Action Center", "Religious Society of Friends", "Peacemakers"], "state" => "District of Columbia", "era" => "1960s", "lat" => 38.9, "lng" => -77.04], "florence-y-carpenter" => ["name" => "Florence Y. Carpenter", "affiliation" => ["Peace Action Center", "Religious Society of Friends"], "state" => "District of Columbia", "era" => "1960s", "lat" => 38.9, "lng" => -77.04], "pearl-c-ewald" => ["name" => "Pearl C. Ewald", "affiliation" => ["Peace Action Center", "Religious Society of Friends"], "state" => "District of Columbia", "era" => "1960s", "lat" => 38.9, "lng" => -77.04]];
    ...
    $approvedDates = ["alfred-hassler" => ["arrest_date" => ["year" => 1944], "release_date" => ["year" => 1945, "month" => 3]], "lawrence-scott" => [], "florence-y-carpenter" => ["arrest_date" => ["year" => 1962], "sentenced_date" => ["year" => 1962]], "pearl-c-ewald" => ["arrest_date" => ["year" => 1962], "sentenced_date" => ["year" => 1962]]];
...
echo "B272-OK\n";
'
run "add-affiliation-prisoners" "B272-OK" "$ADD_CODE" || exit 1
echo "Batch 272 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
