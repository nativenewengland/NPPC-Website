#!/usr/bin/env bash
# Batch 280: 3 missing people from the Anonymous affiliation audit from the affiliation audit.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-280.sh --dry-run
# sudo -u www-data bash database/data/run-batch-280.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-280.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch280.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 280 || ($payload["expected_count"] ?? null) !== 3 || count($payload["entries"] ?? []) !== 3) {
...
foreach ($payload["entries"] as $entry) {
    Validator::make($entry, ["key" => "required|string", "match_names" => "required|array|min:1", "match_names.*" => "required|string", "prisoner.name" => "required|string|max:255", "prisoner.first_name" => "required|string|max:255", "prisoner.last_name" => "required|string|max:255", "prisoner.description" => "required|string", "prisoner.state" => "required|string", "prisoner.era" => "required|in:2010s", "prisoner.in_custody" => "required|boolean|declined", "prisoner.released" => "required|boolean", "prisoner.lat" => "required|numeric|between:-90,90", "prisoner.lng" => "required|numeric|between:-180,180", "prisoner.cases" => "required|array|min:1|max:3", "dates" => "present|array", "case_dates" => "present|array"])->validate();
    ...
    $expected = ["jonathan-cowden" => ["name" => "Jonathan Cowden", "affiliation" => ["Anonymous"], "state" => "Missouri", "era" => "2010s", "lat" => 38.63, "lng" => -90.2], "justin-gray-liverman" => ["name" => "Justin Gray Liverman", "affiliation" => ["Crackas With Attitude"], "state" => "North Carolina", "era" => "2010s", "lat" => 34.72, "lng" => -76.73], "timothy-justen-french" => ["name" => "Timothy Justen French", "affiliation" => ["Anonymous", "NullCrew"], "state" => "Tennessee", "era" => "2010s", "lat" => 36.21, "lng" => -83.29]];
    ...
    $reviewedDates = ["jonathan-cowden" => [["sentenced_date" => ["year" => 2014, "month" => 3, "day" => 27]], ["release_date" => ["year" => 2016, "month" => 7, "day" => 5]]], "justin-gray-liverman" => [["arrest_date" => ["year" => 2016, "month" => 9, "day" => 8], "sentenced_date" => ["year" => 2017, "month" => 9, "day" => 8], "release_date" => ["year" => 2021, "month" => 11, "day" => 22]]], "timothy-justen-french" => [["sentenced_date" => ["year" => 2016, "month" => 11, "day" => 1]]]];
    $reviewedCases = ["jonathan-cowden" => [["charges" => "Computer fraud, 18 U.S.C. 1030", "sentence" => "Fifteen months imposed March 27, 2014. Supporters confirm that he served approximately a year before release. Exact initial custody endpoints remain unverified."], ["charges" => "Alleged violations of supervision conditions following computer-fraud conviction", "sentence" => "Returned to custody in 2015 and held at FCI Victorville Medium I. The Bureau of Prisons records actual release July 5, 2016. The exact start remains unresolved."]], "justin-gray-liverman" => [["charges" => "Conspiracy involving unauthorized computer intrusions, identity theft and telephone harassment", "sentence" => "Five-year prison sentence imposed September 8, 2017, after a January 6 guilty plea. Actual prison service is confirmed by his later first-party biography. BOP records release from federal custody November 22, 2021, with Raleigh residential reentry management as the final location."]], "timothy-justen-french" => [["charges" => "Intentionally damaging a protected computer without authorization, 18 U.S.C. 1030(a)(5)(A)", "sentence" => "Forty-five months imposed November 1, 2016. In a November 19, 2017 account, French described about two years actually spent in prison and a recent transfer to a halfway house in Manchester, Kentucky. Exact prison endpoints are not established."]]];
    ...
    if (isset($entry["photo"])) { throw new \RuntimeException("No verified portrait in this batch."); }
...
echo "B280-OK\n";
'
run "add-affiliation-prisoners" "B280-OK" "$ADD_CODE" || exit 1
echo "Batch 280 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
