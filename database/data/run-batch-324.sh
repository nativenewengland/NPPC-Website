#!/usr/bin/env bash
# Batch 324: Six verified IWW prisoners; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-324.sh --dry-run
# sudo -u www-data bash database/data/run-batch-324.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-324.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch324.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 324 || ($payload["expected_count"] ?? null) !== 6 || count($payload["entries"] ?? []) !== 6) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 324."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["charles-jacobson" => ["name" => "Charles Jacobson", "first_name" => "Charles", "middle_name" => null, "last_name" => "Jacobson", "aka" => null, "description" => "Charles Jacobson was secretary-treasurer of the IWW mine workers union at Virginia, Minnesota. He was jailed in January 1917 during suppression of the northern Minnesota lumber strike, and another organizer took over his duties.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Minnesota", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "j-callahan" => ["name" => "J. Callahan", "first_name" => "J.", "middle_name" => null, "last_name" => "Callahan", "aka" => null, "description" => "J. Callahan appears in the IWW Northwest District prisoner register as a Prosser defendant convicted of criminal syndicalism. The April 1920 report explicitly states that the named defendants were serving their jail sentences.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "ed-lidbery" => ["name" => "Ed Lidbery", "first_name" => "Ed", "middle_name" => null, "last_name" => "Lidbery", "aka" => null, "description" => "Ed Lidbery appears in the IWW Northwest District prisoner register as a Prosser defendant convicted of criminal syndicalism. The April 1920 report explicitly states that the named defendants were serving their jail sentences.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "william-murray" => ["name" => "William Murray", "first_name" => "William", "middle_name" => null, "last_name" => "Murray", "aka" => "Wm. Murray", "description" => "William Murray appears in the IWW Northwest District prisoner register as a Prosser defendant convicted of criminal syndicalism. The April 1920 report explicitly states that the named defendants were serving their jail sentences.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "john-hanson" => ["name" => "John Hanson", "first_name" => "John", "middle_name" => null, "last_name" => "Hanson", "aka" => null, "description" => "John Hanson appears in the IWW Northwest District prisoner register as a Ritzville defendant convicted of criminal syndicalism. The April 1920 report identifies him as serving a custodial sentence.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => null, "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "ralph-bergdorff" => ["name" => "Ralph Bergdorff", "first_name" => "Ralph", "middle_name" => null, "last_name" => "Bergdorff", "aka" => null, "description" => "Ralph Bergdorff appears in the IWW prisoner register as a detainee held for six months at Spokane in 1918. The entry also describes earlier Bellingham confinement and a later return there.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["charles-jacobson" => [], "j-callahan" => [], "ed-lidbery" => [], "william-murray" => [], "john-hanson" => [], "ralph-bergdorff" => []];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["charles-jacobson" => [["arrest_date" => ["year" => 1917, "month" => 1, "day" => 4], "incarceration_date" => ["year" => 1917, "month" => 1, "day" => 4]]], "j-callahan" => [["arrest_date" => ["year" => 1919, "month" => 11, "day" => 16]]], "ed-lidbery" => [["arrest_date" => ["year" => 1919, "month" => 11, "day" => 16]]], "william-murray" => [["arrest_date" => ["year" => 1919, "month" => 11, "day" => 16]]], "john-hanson" => [[]], "ralph-bergdorff" => [["arrest_date" => ["year" => 1918], "incarceration_date" => ["year" => 1918]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["charles-jacobson" => ["Charles Jacobson"], "j-callahan" => ["J. Callahan"], "ed-lidbery" => ["Ed Lidbery"], "william-murray" => ["William Murray", "Wm. Murray"], "john-hanson" => ["John Hanson"], "ralph-bergdorff" => ["Ralph Bergdorff"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["charles-jacobson" => [["charges" => "Municipal charge of lurking and lying in wait with intent to do mischief", "sentence" => "Arrested January 4, 1917 at the union office in Virginia, Minnesota. Received a $100 fine or ninety days in jail; the historical study confirms he served jail time while Richard Mattson took over. Exact release and completed time served are unverified; ninety days is not entered as documented actual duration."]], "j-callahan" => [["charges" => "Criminal syndicalism", "sentence" => "The arrest register dates the Prosser entry November 16, 1919 and records a conviction with a sentence of six months to one year, explicitly being served. Prison admission day, sentencing day, completed time served and eventual release are unknown. Do not use the full nominal sentence as actual duration or assume uninterrupted detention from the arrest date."]], "ed-lidbery" => [["charges" => "Criminal syndicalism", "sentence" => "The arrest register dates the Prosser entry November 16, 1919 and records a conviction with a sentence of six months to one year, explicitly being served. Prison admission day, sentencing day, completed time served and eventual release are unknown. Do not use the full nominal sentence as actual duration or assume uninterrupted detention from the arrest date."]], "william-murray" => [["charges" => "Criminal syndicalism", "sentence" => "The arrest register dates the Prosser entry November 16, 1919 and records a conviction with a sentence of six months to one year, explicitly being served. Prison admission day, sentencing day, completed time served and eventual release are unknown. Do not use the full nominal sentence as actual duration or assume uninterrupted detention from the arrest date."]], "john-hanson" => [["charges" => "Criminal syndicalism", "sentence" => "Convicted at Ritzville and sentenced to six months to one year; the April 1920 register explicitly says he was serving the sentence. No arrest, sentencing, admission or release date supplied. The publication year is not converted into a prison-entry date and the nominal term is not treated as completed service."]], "ralph-bergdorff" => [["charges" => "Detention during the IWW crackdown; precise charge unspecified", "sentence" => "Rearrested at Spokane in 1918 and held in county jail for six months. The entry also describes an earlier one-year Bellingham sentence, a jail escape and reincarceration at Bellingham in January 1919. Only the explicitly documented six-month Spokane interval is counted here; exact dates, possible breaks in custody and eventual release are unknown.", "imprisoned_for_months" => 6]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    if (isset($entry["photo"])) { throw new \RuntimeException("No portrait was verified for batch324."); }
...
echo "B324-OK\n";
'
run "add-affiliation-prisoners" "B324-OK" "$ADD_CODE" || exit 1
echo "Batch 324 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
