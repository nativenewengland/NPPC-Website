#!/usr/bin/env bash
# Batch 260: fill 50 missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch260.json; photos/CREDITS-batch260.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-260.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch260.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 260 || ($payload["expected_count"] ?? null) !== 50 || count($payload["entries"] ?? []) !== 50) {
...
echo "B260-OK\n";
'
run "fill-missing-portraits" "B260-OK" "$UPDATE_CODE" || exit 1
