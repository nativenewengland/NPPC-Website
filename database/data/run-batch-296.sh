#!/usr/bin/env bash
# Batch 296: Josephine Kaczor: four-day Nazareth strike custody and identified portrait.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-296.sh --dry-run
# sudo -u www-data bash database/data/run-batch-296.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-296.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch296.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 296 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
foreach ($payload["entries"] as $entry) {
    Validator::make($entry, ["key" => "required|string", "match_names" => "required|array|min:1", "match_names.*" => "required|string", "prisoner.name" => "required|string|max:255", "prisoner.first_name" => "required|string|max:255", "prisoner.last_name" => "required|string|max:255", "prisoner.description" => "required|string", "prisoner.state" => "required|string", "prisoner.era" => "required|in:1930s", "prisoner.in_custody" => "required|boolean|declined", "prisoner.released" => "required|boolean", "prisoner.lat" => "required|numeric|between:-90,90", "prisoner.lng" => "required|numeric|between:-180,180", "prisoner.cases" => "required|array|min:1|max:3", "dates" => "present|array", "case_dates" => "present|array"])->validate();
    ...
    $expected = ["josephine-kaczor" => ["name" => "Josephine Kaczor", "first_name" => "Josephine", "middle_name" => null, "last_name" => "Kaczor", "aka" => null, "description" => "Josephine Kaczor was a labor organizer and Brookwood Labor College graduate involved in the 1929–1930 hosiery strike in Nazareth, Pennsylvania, against contracts barring union membership. A March 1930 report named her among eleven defendants acquitted of rioting but still assessed court costs; her share was $150. The strikers refused payment, were jailed, and won release after four days on hunger strike. Contemporary coverage also records repeated strike arrests. The dates of her four-day confinement and her birth and death dates remain unverified.", "affiliation" => ["Brookwood Labor College", "Labor movement"], "state" => "Pennsylvania", "era" => "1930s", "lat" => 40.74, "lng" => -75.31, "in_custody" => false, "released" => true, "website" => null, "gender" => "Female", "race" => null, "inmate_number" => null]];
    ...
    $reviewedDates = ["josephine-kaczor" => [[]]];
    $reviewedVitals = ["josephine-kaczor" => []];
    $reviewedCases = ["josephine-kaczor" => [["charges" => "Rioting allegation during the Nazareth hosiery strike; detention for refusing court costs after acquittal", "convicted" => "Acquitted of rioting; assessed $150 in court costs despite the acquittal", "sentence" => "Four days of actual jail confinement after refusing court costs, followed by release after a hunger strike, as reported March 15, 1930. This was not a four-day sentence for a riot conviction. Exact custody dates and the specific facility for this episode remain unverified."]]];
    ...
    $approvedPhotos = ["josephine-kaczor" => ["source_file" => "database/data/photos/josephine-kaczor-labor-age-crop.png", "storage_path" => "prisoners/josephine-kaczor-labor-age-crop.png", "sha256" => "c1907f040c7496330a7c3279860b140ed61947903685a7cfa51e52c5c2a33d07", "source_id" => "portrait"]];
...
echo "B296-OK\n";
'
run "add-affiliation-prisoners" "B296-OK" "$ADD_CODE" || exit 1
echo "Batch 296 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
