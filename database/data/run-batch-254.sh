#!/usr/bin/env bash
# Batch 254: fill eight missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch254.json; photos/CREDITS-batch254.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-254.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch254.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 254 || ($payload["expected_count"] ?? null) !== 8 || count($payload["entries"] ?? []) !== 8) {
...
echo "B254-OK\n";
'
run "fill-missing-activist-photos" "B254-OK" "$UPDATE_CODE" || exit 1
