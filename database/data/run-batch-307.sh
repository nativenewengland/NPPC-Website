#!/usr/bin/env bash
# Batch 307: Three verified Montana Coxeyite detainees; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-307.sh --dry-run
# sudo -u www-data bash database/data/run-batch-307.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-307.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch307.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 307 || ($payload["expected_count"] ?? null) !== 3 || count($payload["entries"] ?? []) !== 3) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["william-hogan" => ["name" => "William Hogan", "first_name" => "William", "middle_name" => null, "last_name" => "Hogan", "aka" => null, "description" => "William Hogan, a Butte teamster, led a Montana contingent of Coxey\u{0027}s Army in the 1894 movement demanding federal relief for unemployment. His contingent seized a Northern Pacific train for its eastward journey and was captured by federal troops.", "affiliation" => ["Coxey\u{0027}s Army"], "state" => "Montana", "era" => "1890s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "john-helme" => ["name" => "John Helme", "first_name" => "John", "middle_name" => null, "last_name" => "Helme", "aka" => "John Helm", "description" => "John Helme was a member of the Montana contingent of Coxey\u{0027}s Army, the 1894 unemployment protest movement. A contemporary jail interview identified him as helping conduct the contingent\u{0027}s affairs while William Hogan was absent.", "affiliation" => ["Coxey\u{0027}s Army"], "state" => "Montana", "era" => "1890s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "p-mcmahon" => ["name" => "P. McMahon", "first_name" => "P.", "middle_name" => null, "last_name" => "McMahon", "aka" => null, "description" => "P. McMahon was a member of the Montana contingent of Coxey\u{0027}s Army, the 1894 unemployment protest movement. A contemporary jail interview identified him as helping conduct the contingent\u{0027}s affairs while William Hogan was absent.", "affiliation" => ["Coxey\u{0027}s Army"], "state" => "Montana", "era" => "1890s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["william-hogan" => [], "john-helme" => [], "p-mcmahon" => []];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["william-hogan" => [["arrest_date" => ["year" => 1894, "month" => 4], "incarceration_date" => ["year" => 1894], "sentenced_date" => ["year" => 1894, "month" => 5, "day" => 14]]], "john-helme" => [["incarceration_date" => ["year" => 1894]]], "p-mcmahon" => [["incarceration_date" => ["year" => 1894]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["william-hogan" => ["William Hogan"], "john-helme" => ["John Helme", "John Helm"], "p-mcmahon" => ["P. McMahon"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["william-hogan" => [["charges" => "Contempt of federal court — interference with a railroad under court injunction", "sentence" => "Sentenced on May 14, 1894 to six months in county jail. The Historical Museum at Fort Missoula reports that he served three months. Exact prison-entry and release dates remain unverified."]], "john-helme" => [["charges" => "Detention following the Montana Coxeyite train seizure; individual charge unverified", "sentence" => "Held at the Helena fairgrounds detention camp in May 1894. A newspaper interview describes confinement extending over several days. Individual sentence and exact release remain unverified."]], "p-mcmahon" => [["charges" => "Detention following the Montana Coxeyite train seizure; individual charge unverified", "sentence" => "Held at the Helena fairgrounds detention camp in May 1894. A newspaper interview describes confinement extending over several days. Individual sentence and exact release remain unverified."]]];
...
echo "B307-OK\n";
'
run "add-affiliation-prisoners" "B307-OK" "$ADD_CODE" || exit 1
echo "Batch 307 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
