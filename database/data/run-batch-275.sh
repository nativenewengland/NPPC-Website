#!/usr/bin/env bash
# Batch 275: 1 missing ACT UP and peace activist from the affiliation audit.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-275.sh --dry-run
# sudo -u www-data bash database/data/run-batch-275.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-275.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch275.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 275 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["jamie-bauer" => ["name" => "Jamie Bauer", "affiliation" => ["ACT UP", "Women\u{0027}s Pentagon Action"], "state" => "New York", "era" => "1980s", "lat" => 42.9, "lng" => -76.86]];
    ...
    $approvedDates = ["jamie-bauer" => ["arrest_date" => ["year" => 1983, "month" => 7, "day" => 30]]];
...
echo "B275-OK\n";
'
run "add-affiliation-prisoners" "B275-OK" "$ADD_CODE" || exit 1
echo "Batch 275 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
