#!/usr/bin/env bash
# Batch 335: Two verified Philippine colonial prisoners; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-335.sh --dry-run
# sudo -u www-data bash database/data/run-batch-335.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-335.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch335.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 335 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 335."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["dionisio-magbuelas" => ["name" => "Dionisio Magbuelas", "first_name" => "Dionisio", "middle_name" => null, "last_name" => "Magbuelas", "aka" => "Papa Isio; Dionisio Seguela; Dionisio Papa y Barlucia", "description" => "Dionisio Magbuelas, known as Papa Isio, led religious and anticolonial resistance in Negros against Spanish and later US rule. After surrendering in 1907, he was imprisoned under the American colonial government and died in Bilibid Prison in Manila in 1911.", "affiliation" => ["Babaylanes", "Philippine independence movement"], "state" => "Philippines", "era" => "1900s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "felipe-salvador" => ["name" => "Felipe Salvador", "first_name" => "Felipe", "middle_name" => null, "last_name" => "Salvador", "aka" => "Apo Ipe; Apong Ipi; Apung Ipe Salvador", "description" => "Felipe Salvador, known as Apo Ipe, led Santa Iglesia, a religious and independence movement in Central Luzon. His resistance to US colonial rule led to capture and a bandolerismo conviction. The colonial court held him responsible for armed raids and killings; he was executed at Bilibid in 1912.", "affiliation" => ["Santa Iglesia", "Philippine independence movement"], "state" => "Philippines", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["dionisio-magbuelas" => ["death_date" => ["year" => 1911]], "felipe-salvador" => ["birthdate" => ["year" => 1870, "month" => 5, "day" => 26], "death_date" => ["year" => 1912, "month" => 4, "day" => 15]]];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["dionisio-magbuelas" => [["arrest_date" => ["year" => 1907, "month" => 8, "day" => 6], "incarceration_date" => ["year" => 1907, "month" => 8, "day" => 6], "sentenced_date" => ["year" => 1907], "death_in_custody_date" => ["year" => 1911]]], "felipe-salvador" => [["arrest_date" => ["year" => 1910, "month" => 7, "day" => 24], "incarceration_date" => ["year" => 1910, "month" => 7, "day" => 24], "death_in_custody_date" => ["year" => 1912, "month" => 4, "day" => 15]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["dionisio-magbuelas" => ["Dionisio Magbuelas", "Papa Isio", "Dionisio Seguela", "Dionisio Papa y Barlucia"], "felipe-salvador" => ["Felipe Salvador", "Apo Ipe", "Apong Ipi", "Apung Ipe Salvador"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["dionisio-magbuelas" => [["charges" => "Bandolerismo (brigandage) under US colonial rule", "sentence" => "After surrender on August 6, 1907, sentenced to death; punishment commuted to life imprisonment in 1907. Died in Bilibid in 1911. The exact death day and institutional transfer dates remain unverified."]], "felipe-salvador" => [["charges" => "Bandolerismo (brigandage), including responsibility for killings during armed raids", "sentence" => "Captured July 24, 1910. The Supreme Court affirmed his death sentence on February 27, 1912; this was judicial review, not the original sentencing date. Executed by hanging at Bilibid, Manila, April 15, 1912. Earlier sedition imprisonment and escape are separate and are not merged into this period."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    $reviewedPhotos = ["dionisio-magbuelas" => ["source_file" => "database/data/photos/dionisio-magbuelas-bandholtz-b335.jpg", "storage_path" => "prisoners/dionisio-magbuelas-bandholtz-b335.jpg", "sha256" => "af9c63be3fb5e81a2db6e7d34eb992c823068d972b9bcbb4cc48d1a2d41c3b37", "source_id" => "isio-photo", "identity_evidence" => "Individually identified archival portrait; Commons existing extraction visually reviewed."], "felipe-salvador" => ["source_file" => "database/data/photos/felipe-salvador-renacimiento-b335.png", "storage_path" => "prisoners/felipe-salvador-renacimiento-b335.png", "sha256" => "78df9a86997b70ad5e7b6e3947940dbc0854e382817f9cf5808ac66ec86b4e01", "source_id" => "salvador-photo", "identity_evidence" => "Captioned reproduction identifies Felipe Apo Ipe Salvador. Exact rectangular crop removes caption strip; retained pixels verified unchanged."]];
...
echo "B335-OK\n";
'
run "add-affiliation-prisoners" "B335-OK" "$ADD_CODE" || exit 1
echo "Batch 335 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
