#!/usr/bin/env bash
# Batch 316: Two verified historical political detainees; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-316.sh --dry-run
# sudo -u www-data bash database/data/run-batch-316.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-316.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch316.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 316 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 316."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["thomas-aloysius-hickey" => ["name" => "Thomas Aloysius Hickey", "first_name" => "Thomas", "middle_name" => "Aloysius", "last_name" => "Hickey", "aka" => "Thomas A. Hickey; T. A. Hickey; Tom Hickey; Red Tom Hickey", "description" => "Thomas Aloysius Hickey, known as Red Tom, was an Irish-born socialist journalist and labor organizer who edited The Rebel in Texas. Authorities detained him during the 1917 anti-conscription crackdown. He spent two days in federal custody before release on bail.", "affiliation" => ["Socialist Party of America", "Knights of Labor", "Western Federation of Miners"], "state" => "Texas", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "raymond-soto-davila" => ["name" => "Raymond Soto Dávila", "first_name" => "Raymond", "middle_name" => null, "last_name" => "Soto Dávila", "aka" => "Raymond Soto Davila; Raymond Soto; Raymond Soto-Davila", "description" => "Raymond Soto Dávila is a Puerto Rican independence activist and historian associated with CUCRE and the Liga Socialista Puertorriqueña. He was confined for civil contempt after refusing to comply with a federal grand jury investigation, including months at the Metropolitan Correctional Center in New York.", "affiliation" => ["Comité Unitario Contra la Represión (CUCRE)", "Liga Socialista Puertorriqueña"], "state" => "New York", "era" => "1980s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["thomas-aloysius-hickey" => ["birthdate" => ["year" => 1869, "month" => 1, "day" => 14], "death_date" => ["year" => 1925, "month" => 5, "day" => 7]], "raymond-soto-davila" => ["birthdate" => ["year" => 1948]]];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["thomas-aloysius-hickey" => [["arrest_date" => ["year" => 1917, "month" => 5, "day" => 17], "incarceration_date" => ["year" => 1917, "month" => 5, "day" => 17], "release_date" => ["year" => 1917, "month" => 5, "day" => 19]]], "raymond-soto-davila" => [["release_date" => ["year" => 1983, "month" => 9, "day" => 25]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["thomas-aloysius-hickey" => ["Thomas Aloysius Hickey", "Thomas A. Hickey", "T. A. Hickey", "Tom Hickey", "Red Tom Hickey"], "raymond-soto-davila" => ["Raymond Soto Dávila", "Raymond Soto Davila", "Raymond Soto", "Raymond Soto-Davila"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["thomas-aloysius-hickey" => [["charges" => "Alleged conspiracy against the government in the anti-conscription crackdown — no prosecution established", "sentence" => "Arrested without a warrant at Brandenburg, Stonewall County, on Thursday, May 17, 1917. His account describes two days held incommunicado in a shared cell before a Saturday hearing and release on $1,000 bail, corresponding to May 19. The hearing was at Abilene. No custodial sentence or conviction is inferred; historical research reports that he was not prosecuted."]], "raymond-soto-davila" => [["charges" => "Civil contempt for noncompliance with a federal grand jury — not a criminal conviction for the acts being investigated", "sentence" => "The proceedings followed December 1982 subpoenas seeking identification exemplars and participation in a lineup. Contemporary April 1983 reporting places him in New York’s Metropolitan Correctional Center; August testimony again identifies him as imprisoned. Torres Maldonado’s source-cited history records almost ten months of confinement and release on September 25, 1983, when the grand jury period was not renewed. The exact arrest and custody-entry dates remain unverified; subpoena and opinion dates are not entered as arrest dates."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    $reviewedPhotos = ["thomas-aloysius-hickey" => null, "raymond-soto-davila" => null];
...
echo "B316-OK\n";
'
run "add-affiliation-prisoners" "B316-OK" "$ADD_CODE" || exit 1
echo "Batch 316 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
