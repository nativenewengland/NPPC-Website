#!/usr/bin/env bash
# Batch 263: fill 100 missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch263.json; photos/CREDITS-batch263.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-263.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch263.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 263 || ($payload["expected_count"] ?? null) !== 100 || count($payload["entries"] ?? []) !== 100) {
...
echo "B263-OK\n";
'
run "fill-missing-portraits" "B263-OK" "$UPDATE_CODE" || exit 1
