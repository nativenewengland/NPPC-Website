#!/usr/bin/env bash
# Batch 314: Two verified environmental activists; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-314.sh --dry-run
# sudo -u www-data bash database/data/run-batch-314.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-314.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch314.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 314 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 314."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["zachary-jenson" => ["name" => "Zachary Jenson", "first_name" => "Zachary", "middle_name" => null, "last_name" => "Jenson", "aka" => "Zach Jenson; Zachary Jensen; Zach Jensen; Ollie", "description" => "Zachary Jenson was an environmental activist prosecuted with Eric McDavid and Lauren Weiner in a Northern California conspiracy case. He spent about six months in Sacramento County Jail, later pleaded guilty to a reduced charge and testified for the prosecution. His eventual sentence credited the jail time already served.", "affiliation" => ["Earth Liberation Front"], "state" => "California", "era" => "2000s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "lauren-weiner" => ["name" => "Lauren Weiner", "first_name" => "Lauren", "middle_name" => null, "last_name" => "Weiner", "aka" => null, "description" => "Lauren Weiner was an environmental activist prosecuted with Eric McDavid and Zachary Jenson in a Northern California conspiracy case. She spent several weeks in Sacramento County Jail before release on bond, pleaded guilty to a reduced charge and testified for the prosecution. Her later sentence included time served and home confinement.", "affiliation" => ["Earth Liberation Front"], "state" => "California", "era" => "2000s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["zachary-jenson" => [], "lauren-weiner" => []];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["zachary-jenson" => [["arrest_date" => ["year" => 2006, "month" => 1, "day" => 13], "incarceration_date" => ["year" => 2006, "month" => 1, "day" => 13], "release_date" => ["year" => 2006, "month" => 7], "sentenced_date" => ["year" => 2008, "month" => 12, "day" => 4]]], "lauren-weiner" => [["arrest_date" => ["year" => 2006, "month" => 1, "day" => 13], "incarceration_date" => ["year" => 2006, "month" => 1, "day" => 13], "release_date" => ["year" => 2006, "month" => 2], "sentenced_date" => ["year" => 2008, "month" => 12, "day" => 11]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["zachary-jenson" => ["Zachary Jenson", "Zach Jenson", "Zachary Jensen", "Zach Jensen"], "lauren-weiner" => ["Lauren Weiner"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["zachary-jenson" => [["charges" => "Federal conspiracy to damage public and private property by fire or explosives — reduced-charge guilty plea", "sentence" => "Arrested January 13, 2006 and held in Sacramento County Jail until release on bail in July 2006. Pleaded guilty July 18, 2006 and cooperated with prosecutors. Sentenced December 4, 2008 to time already served, described as six months, with subsequent supervision. The exact July release day and precise statutory reduced count remain unverified. The later period on pretrial release and supervision is not counted as imprisonment."]], "lauren-weiner" => [["charges" => "Federal conspiracy to damage public and private property by fire or explosives — reduced-charge guilty plea", "sentence" => "Arrested January 13, 2006 and held in Sacramento County Jail until bond release in early February 2006, reported as about three weeks in custody. Pleaded guilty May 30, 2006. Sentenced December 11, 2008 to time served and five months home confinement. Home confinement, pretrial supervision and later supervised release are excluded from the jail period. The exact February release day and precise statutory reduced count remain unverified."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    if (isset($entry["photo"])) { throw new \RuntimeException("No portrait was verified for batch314."); }
...
echo "B314-OK\n";
'
run "add-affiliation-prisoners" "B314-OK" "$ADD_CODE" || exit 1
echo "Batch 314 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
