#!/usr/bin/env bash
# Batch 292: Herbert Hawkins: verified Philadelphia Panther custody and identified portrait.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-292.sh --dry-run
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-292.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch292.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 292 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["herbert-hawkins-philadelphia" => ["name" => "Herbert Hawkins", "affiliation" => ["Black Panther Party", "Black United Liberation Front"], "state" => "Pennsylvania", "era" => "1970s", "lat" => 39.95, "lng" => -75.16, "in_custody" => false, "released" => true, "website" => null, "gender" => "Male", "race" => "Black", "inmate_number" => null]];
    ...
    $reviewedDates = ["herbert-hawkins-philadelphia" => [["arrest_date" => ["year" => 1970, "month" => 8, "day" => 31]]]];
    $reviewedVitals = ["herbert-hawkins-philadelphia" => []];
    $reviewedCases = ["herbert-hawkins-philadelphia" => [["charges" => "Charges arising from the August 31, 1970 Panther-office raid; individual counts unverified", "sentence" => "Two weeks in pretrial custody, followed by release; charges later dismissed. Exact custody endpoints and dismissal date remain unverified."]]];
    ...
    $approvedPhotos = ["herbert-hawkins-philadelphia" => ["source_file" => "database/data/photos/herbert-hawkins-yong-kim-crop.png", "storage_path" => "prisoners/herbert-hawkins-yong-kim-crop.png", "sha256" => "e5084a89ae2fa859901aadffc45cd6410f0abc7fc7cc31d4bb3f90ab552dacf3", "source_id" => "hawkins_interview"]];
...
echo "B292-OK\n";
'
run "add-affiliation-prisoners" "B292-OK" "$ADD_CODE" || exit 1
echo "Batch 292 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
