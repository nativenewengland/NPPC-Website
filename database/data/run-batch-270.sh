#!/usr/bin/env bash
# Batch 270: 5 missing Catonsville and Clamshell activists from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-270.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch270.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 270 || ($payload["expected_count"] ?? null) !== 5 || count($payload["entries"] ?? []) !== 5) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = [
        "david-darst" => [["Catonsville 9"], "Maryland", 39.27, -76.74],
        "arnie-alpert" => [["Clamshell Alliance", "American Friends Service Committee"], "New Hampshire", 42.899, -70.851],
        "harold-marcuse" => [["Clamshell Alliance"], "New Hampshire", 42.899, -70.851],
        "thea-paneth" => [["Clamshell Alliance"], "New Hampshire", 42.899, -70.851],
        "court-dorsey" => [["Clamshell Alliance"], "New Hampshire", 42.899, -70.851],
    ];
    $actual = [$entry["prisoner"]["affiliation"] ?? [], $entry["prisoner"]["state"], $entry["prisoner"]["lat"], $entry["prisoner"]["lng"]];
    if (! isset($expected[$entry["key"]]) || $actual !== $expected[$entry["key"]] || ! $entry["prisoner"]["released"]) { throw new \RuntimeException("Unexpected identity, affiliation, location or custody status."); }
...
echo "B270-OK\n";
'
run "add-affiliation-prisoners" "B270-OK" "$ADD_CODE" || exit 1
echo "Batch 270 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
