#!/usr/bin/env bash
# Batch 258: fill six missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch258.json; photos/CREDITS-batch258.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-258.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch258.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 258 || ($payload["expected_count"] ?? null) !== 6 || count($payload["entries"] ?? []) !== 6) {
...
echo "B258-OK\n";
'
run "fill-missing-portraits" "B258-OK" "$UPDATE_CODE" || exit 1
