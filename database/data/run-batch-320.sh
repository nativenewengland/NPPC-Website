#!/usr/bin/env bash
# Batch 320: Three verified Spokane free-speech prisoners; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-320.sh --dry-run
# sudo -u www-data bash database/data/run-batch-320.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-320.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch320.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 320 || ($payload["expected_count"] ?? null) !== 3 || count($payload["entries"] ?? []) !== 3) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 320."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["samuel-o-chinn" => ["name" => "Samuel O. Chinn", "first_name" => "Samuel", "middle_name" => "O.", "last_name" => "Chinn", "aka" => "S. O. Chinn", "description" => "Samuel O. Chinn was an Industrial Workers of the World member and Spokane union officer jailed during the 1909–1910 free-speech campaign. Contemporary accounts describe prolonged confinement on bread and water. He died in Spokane on March 18, 1910, after leaving jail; the union attributed his deteriorating health to his treatment in custody.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1900s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "henry-bordet" => ["name" => "Henry Bordet", "first_name" => "Henry", "middle_name" => null, "last_name" => "Bordet", "aka" => null, "description" => "Henry Bordet was an Industrial Workers of the World member jailed during the Spokane free-speech fight. His union reported thirty days on bread and water and severe mistreatment in custody. After release he received hospital care in Portland and assistance from Local 437 in Holtville, California. That local recorded his death on February 7, 1911.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1900s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "f-j-ferry" => ["name" => "F. J. Ferry", "first_name" => "F.", "middle_name" => "J.", "last_name" => "Ferry", "aka" => "F. Ferry", "description" => "F. J. Ferry was a Spokane member of Industrial Workers of the World Local 222 and a participant in the free-speech campaign. Arrested in autumn 1909, he was held for thirty-four days at the Franklin School detention site. Contemporary reports describe ill health after release and his subsequent death in a Spokane hospital.", "affiliation" => ["Industrial Workers of the World (IWW)"], "state" => "Washington", "era" => "1900s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["samuel-o-chinn" => ["death_date" => ["year" => 1910, "month" => 3, "day" => 18]], "henry-bordet" => ["death_date" => ["year" => 1911, "month" => 2, "day" => 7]], "f-j-ferry" => ["death_date" => ["year" => 1910, "month" => 4, "day" => 7]]];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["samuel-o-chinn" => [[]], "henry-bordet" => [[]], "f-j-ferry" => [["arrest_date" => ["year" => 1909], "incarceration_date" => ["year" => 1909]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["samuel-o-chinn" => ["Samuel O. Chinn", "S. O. Chinn"], "henry-bordet" => ["Henry Bordet"], "f-j-ferry" => ["F. J. Ferry", "F. Ferry"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["samuel-o-chinn" => [["charges" => "Spokane free-speech campaign detention; precise individual charge unverified", "sentence" => "Confined for more than a month during the 1909–1910 free-speech fight. The March 26 obituary gives inconsistent 35- and 45-day bread-and-water periods, and another report gives thirty days. These establish sustained custody but are not used as an exact sentence or calculated imprisonment duration. Released before his later hospital death; entry and release dates remain unverified."]], "henry-bordet" => [["charges" => "Spokane free-speech campaign detention; precise individual charge unverified", "sentence" => "The January 26, 1911 appeal reports thirty days on bread and water in Spokane custody during the earlier free-speech fight. This is a documented period of actual confinement, not a verified court-imposed sentence or necessarily the full detention. The same account establishes later release and hospital treatment. Exact arrest, entry and release dates remain unresolved."]], "f-j-ferry" => [["charges" => "Spokane free-speech campaign detention; precise individual charge unverified", "sentence" => "Thirty-four days confined at Franklin School during the free-speech fight, with arrest in autumn 1909. Contemporary reporting confirms he left prison before his final illness and death. Exact custody endpoints and the formal sentence remain unverified; the reported confinement is not rounded to one month."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    if (isset($entry["photo"])) { throw new \RuntimeException("No portrait was verified for batch320."); }
...
echo "B320-OK\n";
'
run "add-affiliation-prisoners" "B320-OK" "$ADD_CODE" || exit 1
echo "Batch 320 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
