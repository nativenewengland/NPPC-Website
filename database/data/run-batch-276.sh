#!/usr/bin/env bash
# Batch 276: 1 missing Latin King and Queen Nation activist from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-276.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch276.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 276 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
$checkCase = function ($case) {
    ...
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "institution_id" => "sometimes|string", "imprisoned_for_months" => "required|integer|in:8"])->validate();
};
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["antonio-fernandez-king-tone" => ["name" => "Antonio Fernandez", "affiliation" => ["Almighty Latin King & Queen Nation"], "state" => "New York", "era" => "1990s", "lat" => 40.85, "lng" => -73.9]];
    ...
    $approvedDates = ["antonio-fernandez-king-tone" => ["arrest_date" => ["year" => 1995, "month" => 5, "day" => 12], "release_date" => ["year" => 1996, "month" => 3]]];
...
echo "B276-OK\n";
'
run "add-affiliation-prisoners" "B276-OK" "$ADD_CODE" || exit 1
echo "Batch 276 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
