...
#   sudo -u www-data bash database/data/run-chronological-portraits-2026-09-11.sh --dry-run
#   sudo -u www-data bash database/data/run-chronological-portraits-2026-09-11.sh
# Provenance and rights: fixes/chronological-portraits-2026-09-11.json; photos/CREDITS-chronological-portraits-2026-09-11.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-chronological-portraits-2026-09-11.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/chronological-portraits-2026-09-11.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== "chronological-portraits-2026-09-11" || ($payload["expected_count"] ?? null) !== 100 || count($payload["entries"] ?? []) !== 100) {
...
echo "CHRONO100-OK\n";
'
run "fill-missing-portraits" "CHRONO100-OK" "$UPDATE_CODE" || exit 1
