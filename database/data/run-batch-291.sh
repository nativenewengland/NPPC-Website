#!/usr/bin/env bash
# Batch 291: Two missing New York Panthers with three documented pretrial custody cases.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-291.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch291.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 291 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
$checkCase = function ($case) {
    ...
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "institution_id" => "sometimes|string", "imprisoned_for_months" => "sometimes|integer|min:1|max:4"])->validate();
};
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["joudon-ford" => ["name" => "Joudon M. Ford", "affiliation" => ["Black Panther Party"], "state" => "New York", "era" => "1960s", "lat" => 40.71, "lng" => -74.01, "in_custody" => false, "released" => true, "website" => null, "gender" => "Male", "race" => "Black", "inmate_number" => null], "tyronne-smith-new-york-panther" => ["name" => "Tyronne Smith", "affiliation" => ["Black Panther Party"], "state" => "New York", "era" => "1960s", "lat" => 40.71, "lng" => -74.01, "in_custody" => false, "released" => false, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewedDates = ["joudon-ford" => [["arrest_date" => ["year" => 1969, "month" => 1, "day" => 24]], []], "tyronne-smith-new-york-panther" => [["arrest_date" => ["year" => 1969, "month" => 1, "day" => 24]]]];
    $reviewedVitals = ["joudon-ford" => ["birthdate" => ["year" => 1950], "death_date" => ["year" => 2024]], "tyronne-smith-new-york-panther" => []];
    $reviewedCases = ["joudon-ford" => [["charges" => "Attempting to carry a concealed weapon onto an airplane", "sentence" => "Pretrial detention; disposition unverified. Ford recalled one month in federal detention at West Street after the January 24, 1969 airport arrest. He acknowledged possessing guns but denied intending a hijacking.", "imprisoned_for_months" => 1], ["charges" => "Attempted murder, according to Ford’s retrospective account", "sentence" => "Ford recalled four months of pretrial detention at Rikers Island in 1969. He maintained his innocence. No verdict, sentence or exact release date has been verified.", "imprisoned_for_months" => 4, "institution_id" => "1b84c078-2ee3-4856-b40f-1a2049f0f067"]], "tyronne-smith-new-york-panther" => [["charges" => "Attempting to carry a concealed weapon onto an airplane", "sentence" => "Pretrial custody continued after bail was reduced from $50,000 to $10,000. No conviction, sentence, total time served or exact release date has been established."]]];
    ...
    $approvedPhotos = ["joudon-ford" => ["source_file" => "database/data/photos/joudon-ford-bev-grant.jpg", "storage_path" => "prisoners/joudon-ford-bev-grant.jpg", "sha256" => "5b5be2f6b970fcfcd617bc349e8076662c68e04615488db0385608c960a374ae", "source_id" => "portrait"], "tyronne-smith-new-york-panther" => null];
...
echo "B291-OK\n";
'
run "add-affiliation-prisoners" "B291-OK" "$ADD_CODE" || exit 1
echo "Batch 291 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
