#!/usr/bin/env bash
# Batch 278: 4 missing New Kensington defendants from the affiliation audit.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-278.sh --dry-run
# sudo -u www-data bash database/data/run-batch-278.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-278.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch278.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 278 || ($payload["expected_count"] ?? null) !== 4 || count($payload["entries"] ?? []) !== 4) {
...
$checkCase = function ($case) {
    ...
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "institution_id" => "sometimes|string", "imprisoned_for_months" => "required|integer|in:10"])->validate();
};
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["harry-w-truitt-jr" => ["name" => "Harry W. Truitt Jr.", "affiliation" => ["Progressive Party"], "state" => "Pennsylvania", "era" => "1950s", "lat" => 40.57, "lng" => -79.77], "john-f-allen" => ["name" => "John F. Allen", "affiliation" => [], "state" => "Pennsylvania", "era" => "1950s", "lat" => 40.57, "lng" => -79.77], "lester-peay" => ["name" => "Lester Peay", "affiliation" => [], "state" => "Pennsylvania", "era" => "1950s", "lat" => 40.57, "lng" => -79.77], "robert-t-smith" => ["name" => "Robert T. Smith", "affiliation" => [], "state" => "Pennsylvania", "era" => "1950s", "lat" => 40.57, "lng" => -79.77]];
    ...
    if ($entry["case_dates"] !== ["arrest_date" => ["year" => 1950, "month" => 3, "day" => 18]] || $entry["dates"] !== []) { throw new \RuntimeException("Unverified date."); }
    $approvedPhotos = ["harry-w-truitt-jr" => ["source_file" => "database/data/photos/harry-w-truitt-jr-national-guardian.png", "storage_path" => "prisoners/harry-w-truitt-jr-national-guardian.png", "sha256" => "441b2e94f46dcba1386193233e6909b72a7767f17c5ccf7025e307da0c6c4312"]];
...
echo "B278-OK\n";
'
run "add-affiliation-prisoners" "B278-OK" "$ADD_CODE" || exit 1
echo "Batch 278 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
