#!/usr/bin/env bash
# Batch 267: 6 peace-list activists with confirmed multi-day US custody.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-267.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch267.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 267 || ($payload["expected_count"] ?? null) !== 6 || count($payload["entries"] ?? []) !== 6) {
...
$checkCase = function ($case) {
    if (array_diff(array_keys($case), ["charges", "sentence", "convicted", "imprisoned_for_months"])) { throw new \RuntimeException("Unexpected case field."); }
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "imprisoned_for_months" => "sometimes|integer|min:1|max:1200"])->validate();
};
$assets = [];
...
echo "B267-OK\n";
'
run "add-peace-activists-with-multiday-us-custody" "B267-OK" "$ADD_CODE" || exit 1
echo "Batch 267 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
