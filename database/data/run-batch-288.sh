#!/usr/bin/env bash
# Batch 288: One missing Black Liberation Movement prison organizer.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-288.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch288.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 288 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["kijana-tashiri-askari" => ["name" => "Kijana Tashiri Askari", "affiliation" => ["Black Liberation Movement"], "state" => "California", "era" => "1990s", "lat" => 41.76, "lng" => -124.2, "in_custody" => false, "released" => false, "website" => null, "gender" => "Male", "race" => "Black", "inmate_number" => "H54077"]];
    ...
    $reviewedDates = ["kijana-tashiri-askari" => [[]]];
    $reviewedVitals = ["kijana-tashiri-askari" => []];
    $reviewedCases = ["kijana-tashiri-askari" => [["charges" => "Indefinite security-housing placement and restrictions on Black August political correspondence", "sentence" => "Administrative confinement, not a new criminal sentence. Askari dates his solitary confinement from August 12, 1994 to November 4, 2015, when he entered Kern Valley general population. He connects it to Black August observance. The 2010 federal order confirms indefinite Pelican Bay SHU placement following gang validation and use of political correspondence as gang evidence. It denied officials summary judgment on censorship claims; it did not invalidate his entire confinement. Transfer from solitary was not release from prison. Original criminal convictions are outside this activism case.", "institution_id" => "4d17ee62-74b8-4c04-9e72-a9d58580d5a8"]]];
    ...
    if (array_key_exists("photo", $entry)) { throw new \RuntimeException("No portrait approved for this batch."); }
...
echo "B288-OK\n";
'
run "add-affiliation-prisoners" "B288-OK" "$ADD_CODE" || exit 1
echo "Batch 288 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
