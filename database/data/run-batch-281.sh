#!/usr/bin/env bash
# Batch 281: Two missing Anti-Rent War prisoners with verified multi-day custody.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-281.sh --dry-run
# sudo -u www-data bash database/data/run-batch-281.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-281.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch281.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 281 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
foreach ($payload["entries"] as $entry) {
    Validator::make($entry, ["key" => "required|string", "match_names" => "required|array|min:1", "match_names.*" => "required|string", "prisoner.name" => "required|string|max:255", "prisoner.first_name" => "required|string|max:255", "prisoner.last_name" => "required|string|max:255", "prisoner.description" => "required|string", "prisoner.state" => "required|string", "prisoner.era" => "required|in:1840s", "prisoner.in_custody" => "required|boolean|declined", "prisoner.released" => "required|boolean", "prisoner.lat" => "required|numeric|between:-90,90", "prisoner.lng" => "required|numeric|between:-180,180", "prisoner.cases" => "required|array|min:1|max:3", "dates" => "present|array", "case_dates" => "present|array"])->validate();
    ...
    $expected = ["william-vroman" => ["name" => "William Vroman", "affiliation" => ["Anti-Rent War"], "state" => "New York", "era" => "1840s", "lat" => 42.47, "lng" => -74.44], "john-j-warner" => ["name" => "John J. Warner", "affiliation" => ["Anti-Rent War"], "state" => "New York", "era" => "1840s", "lat" => 42.47, "lng" => -74.44]];
    ...
    $reviewedDates = ["william-vroman" => [["arrest_date" => ["year" => 1845, "month" => 8]]], "john-j-warner" => [["arrest_date" => ["year" => 1845, "month" => 8]]]];
    $reviewedCases = ["william-vroman" => [["charges" => "Detention during the 1845 Anti-Rent War crackdown; formal charge not established", "sentence" => "Held for several days in August 1845. Mayham names Vroman among the prisoners taken to Ira B. Rose\u{0027}s tavern at Gilboa and describes him still in custody on the following Sunday. Vroman later disputed an account that the prisoners had been paroled over the weekend. The account describes lawyer Thomas Smith obtaining the remaining prisoners\u{0027} release. Exact admission and release dates and any formal sentence are not established."]], "john-j-warner" => [["charges" => "Detention during the 1845 Anti-Rent War crackdown; formal charge not established", "sentence" => "Confined in the Brimstone church during the August 1845 crackdown, then held at Schoharie for a week, according to his daughter\u{0027}s recollection published in 1906. The week refers to the Schoharie confinement, not the total including the church. Exact arrest and release days, the formal charge and any imposed sentence remain unverified."]]];
...
echo "B281-OK\n";
'
run "add-affiliation-prisoners" "B281-OK" "$ADD_CODE" || exit 1
echo "Batch 281 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
