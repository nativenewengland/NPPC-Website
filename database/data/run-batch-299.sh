#!/usr/bin/env bash
# Batch 299: Three verified Catholic Worker detainees; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-299.sh --dry-run
# sudo -u www-data bash database/data/run-batch-299.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-299.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch299.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 299 || ($payload["expected_count"] ?? null) !== 3 || count($payload["entries"] ?? []) !== 3) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["nicole-d-entremont" => ["name" => "Nicole d’Entremont", "first_name" => "Nicole", "middle_name" => null, "last_name" => "d’Entremont", "aka" => null, "description" => "Nicole d’Entremont was a Catholic Worker editor jailed after an antiwar sit-down at New York’s Armed Forces Day parade in 1966.", "affiliation" => ["Catholic Worker"], "state" => "New York", "era" => "1960s", "lat" => 40.712749, "lng" => -74.005994, "in_custody" => false, "released" => true, "website" => null, "gender" => "Female", "race" => null, "inmate_number" => null], "dianne-feeley" => ["name" => "Dianne Feeley", "first_name" => "Dianne", "middle_name" => null, "last_name" => "Feeley", "aka" => null, "description" => "Dianne Feeley participated in Catholic Worker antiwar actions before joining the Young Socialist Alliance in 1967. She later became a labor and feminist organizer.", "affiliation" => ["Catholic Worker"], "state" => "New York", "era" => "1960s", "lat" => 40.712749, "lng" => -74.005994, "in_custody" => false, "released" => true, "website" => null, "gender" => "Female", "race" => null, "inmate_number" => null], "patricia-rusk" => ["name" => "Patricia Rusk", "first_name" => "Patricia", "middle_name" => null, "last_name" => "Rusk", "aka" => null, "description" => "Patricia Rusk participated in the Assembly of Unrepresented People’s antiwar sit-down in Washington, D.C. She described her imprisonment in the Catholic Worker.", "affiliation" => ["Catholic Worker"], "state" => "District of Columbia", "era" => "1960s", "lat" => 38.90472, "lng" => -77.01639, "in_custody" => false, "released" => true, "website" => null, "gender" => "Female", "race" => null, "inmate_number" => null]];
    ...
    $reviewedDates = ["nicole-d-entremont" => [["arrest_date" => ["year" => 1966], "incarceration_date" => ["year" => 1966]]], "dianne-feeley" => [["arrest_date" => ["year" => 1966], "incarceration_date" => ["year" => 1966]]], "patricia-rusk" => [["arrest_date" => ["year" => 1965, "month" => 8, "day" => 9], "incarceration_date" => ["year" => 1965, "month" => 8, "day" => 9]]]];
    $reviewedVitals = ["nicole-d-entremont" => [], "dianne-feeley" => [], "patricia-rusk" => []];
    $reviewedNames = ["nicole-d-entremont" => ["Nicole d’Entremont", "Nicole d\u{0027}Entremont", "Nicole Dentremont", "Nicole D Entremont"], "dianne-feeley" => ["Dianne Feeley", "Diane Feeley"], "patricia-rusk" => ["Patricia Rusk", "Pat Rusk"]];
    $reviewedCases = ["nicole-d-entremont" => [["charges" => "Antiwar sit-down; individual statutory charge not verified", "sentence" => "Five-day jail sentence; three additional days of pretrial custody after refusing bail. A separate contemporary report confirms her return from the five-day term. Exact custody endpoints remain unverified.", "institution_id" => "c9293221-454b-4f1c-a39b-c946957f5a41"]], "dianne-feeley" => [["charges" => "Antiwar sit-down; individual statutory charge not verified", "sentence" => "Three days of pretrial custody after refusing bail; subsequently reported in jail serving a five-day sentence. Completion of that additional term and exact release date remain unverified.", "institution_id" => "c9293221-454b-4f1c-a39b-c946957f5a41"]], "patricia-rusk" => [["charges" => "Antiwar sit-down; individual statutory charge not verified", "sentence" => "Served five days in D.C. Jail following the August 9, 1965 antiwar sit-down. Exact release date and individual charge wording remain unverified."]]];
...
echo "B299-OK\n";
'
run "add-affiliation-prisoners" "B299-OK" "$ADD_CODE" || exit 1
echo "Batch 299 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
