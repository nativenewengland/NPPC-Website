#!/usr/bin/env bash
# Batch 259: fill ten missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch259.json; photos/CREDITS-batch259.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-259.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch259.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 259 || ($payload["expected_count"] ?? null) !== 10 || count($payload["entries"] ?? []) !== 10) {
...
echo "B259-OK\n";
'
run "fill-missing-portraits" "B259-OK" "$UPDATE_CODE" || exit 1
