#!/usr/bin/env bash
# Chronological portrait research: fill 100 missing prisoner photos; preserve every attached photo.
# This self-contained photo-only batch has no dependencies on other batches.
#   sudo -u www-data bash database/data/run-chronological-portraits-2-2026-09-11.sh --dry-run
#   sudo -u www-data bash database/data/run-chronological-portraits-2-2026-09-11.sh
# Provenance and rights: fixes/chronological-portraits-2-2026-09-11.json; photos/CREDITS-chronological-portraits-2-2026-09-11.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-chronological-portraits-2-2026-09-11.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/chronological-portraits-2-2026-09-11.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== "chronological-portraits-2-2026-09-11" || ($payload["expected_count"] ?? null) !== 100 || count($payload["entries"] ?? []) !== 100) {
...
$changed = DB::transaction(function () use ($payload, $dryRun, $disk, $assets) {
    ...
    foreach ($payload["entries"] as $entry) {
        ...
        $plans[] = ["record" => $record, "original" => $original, "asset" => $asset, "before" => $before];
    }
    if (count($plans) !== 100) { throw new \RuntimeException("Expected 100 empty photo fields; no database changes applied."); }
    ...
    foreach ($plans as $plan) {
        ...
        // Guarded query-builder update preserves timestamps and avoids model hooks.
        // A concurrent photo change rolls back the transaction, never overwrites it.
        $count = DB::table("prisoners")->where("id", $record->id)
    ...
    foreach ($plans as $plan) {
        if ($dryRun) { continue; }
        $after = (array) DB::table("prisoners")->where("id", $plan["record"]->id)->sole();
        $before = $plan["before"];
        unset($before["photo"], $after["photo"]);
        if ($before != $after) { throw new \RuntimeException("Unexpected non-photo field change."); }
    }
    return count($plans);
});
if (! $dryRun && $changed > 0) {
...
echo "CHRONO200-OK\n";
'
run "fill-missing-portraits" "CHRONO200-OK" "$UPDATE_CODE" || exit 1
