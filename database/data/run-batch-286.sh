#!/usr/bin/env bash
# Batch 286: Two missing BLA-associated prisoners; two verified historical custody cases.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-286.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch286.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 286 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["melvin-rema-kearney" => ["name" => "Melvin “Rema” Kearney", "affiliation" => ["Black Liberation Army"], "state" => "New York", "era" => "1970s", "lat" => 40.69, "lng" => -73.99, "in_custody" => false, "released" => false, "website" => null], "pedro-mario-monges" => ["name" => "Pedro Mario Monges", "affiliation" => ["Black Liberation Army"], "state" => "New York", "era" => "1970s", "lat" => 40.69, "lng" => -73.99, "in_custody" => false, "released" => false, "website" => null]];
    ...
    $reviewedDates = ["melvin-rema-kearney" => [["death_in_custody_date" => ["year" => 1975, "month" => 5, "day" => 25]]], "pedro-mario-monges" => [["sentenced_date" => ["year" => 1973]]]];
    $reviewedVitals = ["melvin-rema-kearney" => ["death_date" => ["year" => 1975, "month" => 5, "day" => 25]], "pedro-mario-monges" => []];
    $reviewedCases = ["melvin-rema-kearney" => [["charges" => "Attempted murder of police; pretrial detention at Brooklyn House of Detention for Men", "sentence" => "Contemporary reporting describes a pending trial over an alleged 1973 attack on police. The state chronology documents Kearney in custody on August 15, 1974 and May 25, 1975. He died in a fall during the latter attempted escape. No conviction, imposed sentence, exact arrest date or uninterrupted day total is inferred. The death ended this custody; it was not a release."]], "pedro-mario-monges" => [["charges" => "Federal bank robbery, 18 U.S.C. § 2113(a); February 9, 1973 Bronx bank robbery", "convicted" => "Guilty plea to Count 1 (bank robbery)", "sentence" => "Eighteen years, as individually stated for Monges in footnote 2 of United States v. Estremera. A university archival catalog dates his imprisonment to 1973; city records place him in Brooklyn custody by September 24, 1973, and the state chronology documents confinement in 1974 and May 1975. In Wilson v. Beame (1974), he was a plaintiff challenging administrative-segregation restrictions while additional escape charges were pending. The eighteen-year penalty is not a verified time-served total. His release date and subsequent status have not been established."]]];
    ...
    $approvedPhotos = ["melvin-rema-kearney" => ["source_file" => "database/data/photos/melvin-rema-kearney-newsletter-1975-crop.png", "storage_path" => "prisoners/melvin-rema-kearney-newsletter-1975-crop.png", "sha256" => "98603735aab263dcdc91a66ab884f764e8730f480a5becc55ef8a4954f3b025d", "source_id" => "dragon"], "pedro-mario-monges" => null];
...
echo "B286-OK\n";
'
run "add-affiliation-prisoners" "B286-OK" "$ADD_CODE" || exit 1
echo "Batch 286 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
