#!/usr/bin/env bash
# Batch 330: Two verified military war resisters; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-330.sh --dry-run
# sudo -u www-data bash database/data/run-batch-330.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-330.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch330.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 330 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 330."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["ryan-johnson" => ["name" => "Ryan Johnson", "first_name" => "Ryan", "middle_name" => null, "last_name" => "Johnson", "aka" => null, "description" => "Ryan Johnson was a US Army soldier who refused deployment to Iraq and sought refuge in Canada. He helped organize a Toronto chapter of Iraq Veterans Against the War before returning to the United States and serving a military prison term.", "affiliation" => ["Iraq Veterans Against the War"], "state" => "California", "era" => "2010s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null], "patrick-brendan-hart" => ["name" => "Patrick Brendan Hart", "first_name" => "Patrick", "middle_name" => "Brendan", "last_name" => "Hart", "aka" => "Patrick Hart; Pat Hart", "description" => "Patrick Hart was a US Army sergeant who publicly opposed the Iraq War while seeking refuge in Canada. He later returned voluntarily and was imprisoned following military prosecution for his refusal to deploy.", "affiliation" => ["Iraq War resistance"], "state" => "Kentucky", "era" => "2010s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["ryan-johnson" => [], "patrick-brendan-hart" => []];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["ryan-johnson" => [["incarceration_date" => ["year" => 2016], "sentenced_date" => ["year" => 2016], "release_date" => ["year" => 2017]]], "patrick-brendan-hart" => [["incarceration_date" => ["year" => 2011], "sentenced_date" => ["year" => 2011, "month" => 3]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["ryan-johnson" => ["Ryan Johnson"], "patrick-brendan-hart" => ["Patrick Brendan Hart", "Patrick Hart", "Pat Hart"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["ryan-johnson" => [["charges" => "Military prosecution following unauthorized absence and refusal of Iraq deployment; exact plea specifications unverified", "sentence" => "A supporter reported his court-martial, ten-month sentence and actual imprisonment in October 2016. A March 2017 campaign update describes continuing confinement at Miramar and visits from his wife. An October 2017 report confirms release and resettlement. The earlier predicted May release is not entered as an actual date."]], "patrick-brendan-hart" => [["charges" => "Desertion following refusal of Iraq deployment", "sentence" => "A March 27, 2011 support notice identifies him in Christian County Jail after sentencing the previous week to twenty-five months. Later reporting confirms completed military imprisonment and release. Conflicting chronology prevents entering a reliable release date or actual total served. Military discharge is separate from jail release."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    if (isset($entry["photo"])) { throw new \RuntimeException("No portrait was verified for batch330."); }
...
echo "B330-OK\n";
'
run "add-affiliation-prisoners" "B330-OK" "$ADD_CODE" || exit 1
echo "Batch 330 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
