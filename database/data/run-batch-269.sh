#!/usr/bin/env bash
# Batch 269: 81 missing Heart Mountain draft resisters from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-269.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch269.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 269 || ($payload["expected_count"] ?? null) !== 81 || count($payload["entries"] ?? []) !== 81) {
...
foreach ($payload["entries"] as $entry) {
    ...
    if (($entry["prisoner"]["affiliation"] ?? []) !== ["Heart Mountain draft resistance"]) { throw new \RuntimeException("Unexpected movement affiliation."); }
    if (($entry["prisoner"]["state"] ?? null) !== "Wyoming" || $entry["prisoner"]["lat"] !== 44.67 || $entry["prisoner"]["lng"] !== -108.95) { throw new \RuntimeException("Unexpected map location."); }
    if (isset($entry["case_dates"]["death_in_custody_date"]) !== ($entry["key"] === "iriye-fred-homi") || $entry["prisoner"]["released"] !== ($entry["key"] !== "iriye-fred-homi")) { throw new \RuntimeException("Unexpected historical custody status."); }
...
echo "B269-OK\n";
'
run "add-heart-mountain-draft-resisters" "B269-OK" "$ADD_CODE" || exit 1
echo "Batch 269 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
