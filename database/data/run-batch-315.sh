#!/usr/bin/env bash
# Batch 315: One verified Elaine legal-support detainee; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-315.sh --dry-run
# sudo -u www-data bash database/data/run-batch-315.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-315.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch315.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 315 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
if ($precisionProbe->computeImprisonedForDays() !== null) { throw new \RuntimeException("Pull the accompanying partial-date counter fix before applying batch 315."); }
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["ocier-s-bratton" => ["name" => "Ocier S. Bratton", "first_name" => "Ocier", "middle_name" => "S.", "last_name" => "Bratton", "aka" => "Ocier Bratton; O. S. Bratton", "description" => "Ocier S. Bratton helped arrange legal representation for Black sharecroppers seeking fair payment for their cotton in Phillips County, Arkansas. The son and agent of attorney Ulysses S. Bratton, he was arrested during the Elaine Massacre and spent a month in jail. His release required a guarded departure because of threats of lynching.", "affiliation" => ["Elaine Massacre"], "state" => "Arkansas", "era" => "1910s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => null, "race" => null, "inmate_number" => null]];
    ...
    $reviewed = ["ocier-s-bratton" => []];
    if ($entry["dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed dates."); }
    $reviewed = ["ocier-s-bratton" => [["arrest_date" => ["year" => 1919, "month" => 10, "day" => 1], "incarceration_date" => ["year" => 1919, "month" => 10, "day" => 1], "release_date" => ["year" => 1919, "month" => 10, "day" => 31]]]];
    if ($entry["case_dates"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed case_dates."); }
    $reviewed = ["ocier-s-bratton" => ["Ocier S. Bratton", "Ocier Bratton", "O. S. Bratton"]];
    if ($entry["match_names"] !== $reviewed[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed match_names."); }
    $reviewedCases = ["ocier-s-bratton" => [["charges" => "Murder accusation followed by a barratry indictment — no conviction established", "sentence" => "Arrested October 1, 1919 while obtaining legal-retainer agreements from sharecroppers near Elaine. Held in Phillips County Jail in Helena until October 31, 1919. Scipio Jones’s court petition states that a barratry indictment replaced the murder accusation on the release day and describes the judge assisting his secret departure to avoid a lynch mob. This records pretrial detention; no prison sentence or adjudicated guilt is inferred. The later disposition of the barratry indictment remains unverified."]]];
    if ($entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unexpected reviewed cases."); }
    $reviewedPhotos = ["ocier-s-bratton" => ["source_file" => "database/data/photos/ocier-bratton-family-1913-crop.png", "storage_path" => "prisoners/ocier-bratton-family-1913-crop.png", "sha256" => "58699605ad971be7216a2962b186225ba3e64a982bbd58cc0683fa650bd8786e", "source_ids" => ["family-photo"], "identity_evidence" => "Explicit eight-person left-to-right caption on printed55 / PDF63; fourth person is Ocier, seated in a light jacket. Source page and extracted image visually reviewed.", "credit" => "Bratton family photograph,1913; reproduced in Richard (Sharp) Martin, My Father’s House: An Abridgment (2023), p.55. Photographer unidentified; rights status unverified.", "changes" => "Exact rectangular crop of the embedded674x483 PNG: left254,top94,right365,bottom314; output111x220. Retained pixels unchanged; no enhancement or redraw."]];
...
echo "B315-OK\n";
'
run "add-affiliation-prisoners" "B315-OK" "$ADD_CODE" || exit 1
echo "Batch 315 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
