#!/usr/bin/env bash
# Batch 271: 5 missing CNVA Everyman defendants from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-271.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch271.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 271 || ($payload["expected_count"] ?? null) !== 5 || count($payload["entries"] ?? []) !== 5) {
...
$checkCase = function ($case) {
    ...
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "institution_id" => "sometimes|string", "imprisoned_for_months" => "sometimes|integer|in:6"])->validate();
};
...
foreach ($payload["entries"] as $entry) {
    ...
    $allowedKeys = ["samuel-reynier-tyson", "walter-chaffee", "barton-stone", "robert-robbins", "roger-moss"];
    $affiliations = $entry["key"] === "samuel-reynier-tyson" ? ["Committee for Nonviolent Action", "Fellowship of Reconciliation"] : ["Committee for Nonviolent Action"];
    if (! in_array($entry["key"], $allowedKeys, true) || ($entry["prisoner"]["affiliation"] ?? []) !== $affiliations || ! $entry["prisoner"]["released"] || $entry["prisoner"]["state"] !== "California" || $entry["prisoner"]["lat"] !== 37.77 || $entry["prisoner"]["lng"] !== -122.42) { throw new \RuntimeException("Unexpected identity, affiliation, location or custody status."); }
    $months = $entry["prisoner"]["cases"][0]["imprisoned_for_months"] ?? null;
    if ($months !== ($entry["key"] === "samuel-reynier-tyson" ? 6 : null)) { throw new \RuntimeException("Unverified actual duration."); }
...
echo "B271-OK\n";
'
run "add-affiliation-prisoners" "B271-OK" "$ADD_CODE" || exit 1
echo "Batch 271 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
