#!/usr/bin/env bash
# Batch 303: Two verified U.S. colonial-custody detainees; accumulate toward 100-person PR.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-303.sh --dry-run
# sudo -u www-data bash database/data/run-batch-303.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-303.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch303.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 303 || ($payload["expected_count"] ?? null) !== 2 || count($payload["entries"] ?? []) !== 2) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["guillermo-capadocia" => ["name" => "Guillermo Capadocia", "first_name" => "Guillermo", "middle_name" => null, "last_name" => "Capadocia", "aka" => "Guillermo Capacodia; Guillermo Capacudia", "description" => "Guillermo Capadocia was a Filipino labor organizer and Communist Party leader prosecuted for political speeches under the U.S. colonial government. Contemporary reports identify him among the organizers imprisoned at Bilibid in 1933–1934.", "affiliation" => ["Communist Party of the Philippines"], "state" => null, "era" => "1930s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => "Male", "race" => null, "inmate_number" => null], "juan-feleo" => ["name" => "Juan Feleo", "first_name" => "Juan", "middle_name" => null, "last_name" => "Feleo", "aka" => null, "description" => "Juan Feleo was a Filipino peasant organizer and Communist activist imprisoned under the U.S. colonial government. He was prosecuted for a speech at a farmers’ grievance meeting in San Miguel, Bulacan, and contemporary reporting describes his imprisonment in 1933–1934.", "affiliation" => ["Communist Party of the Philippines"], "state" => null, "era" => "1930s", "lat" => null, "lng" => null, "in_custody" => false, "released" => true, "website" => null, "gender" => "Male", "race" => null, "inmate_number" => null]];
    ...
    $reviewedDates = ["guillermo-capadocia" => [["incarceration_date" => ["year" => 1933]]], "juan-feleo" => [["incarceration_date" => ["year" => 1933]]]];
    $reviewedVitals = ["guillermo-capadocia" => [], "juan-feleo" => []];
    $reviewedNames = ["guillermo-capadocia" => ["Guillermo Capadocia", "Guillermo Capacodia", "Guillermo Capacudia"], "juan-feleo" => ["Juan Feleo"]];
    $reviewedCases = ["guillermo-capadocia" => [["charges" => "Sedition: section 8 of Act No. 292", "sentence" => "Imprisoned at Bilibid following the colonial sedition proceedings. An additional internal-banishment order was separate from imprisonment. Exact custody endpoints and total time served remain unverified."]], "juan-feleo" => [["charges" => "Inciting sedition: article 142 of the Revised Penal Code", "sentence" => "Four years, nine months and eleven days plus a 500-peso fine, affirmed October 14, 1933 for the September 29, 1932 speech. Contemporary reporting confirms prison entry in 1933 and a subsequent release campaign. Exact release and actual duration remain unverified."]]];
...
echo "B303-OK\n";
'
run "add-affiliation-prisoners" "B303-OK" "$ADD_CODE" || exit 1
echo "Batch 303 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
