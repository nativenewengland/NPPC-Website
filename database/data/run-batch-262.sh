#!/usr/bin/env bash
# Batch 262: fill 100 missing prisoner photos; preserve every attached photo.
...
# Provenance and rights: fixes/batch262.json; photos/CREDITS-batch262.md.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-262.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch262.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 262 || ($payload["expected_count"] ?? null) !== 100 || count($payload["entries"] ?? []) !== 100) {
...
echo "B262-OK\n";
'
run "fill-missing-portraits" "B262-OK" "$UPDATE_CODE" || exit 1
