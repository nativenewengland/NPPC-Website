#!/usr/bin/env bash
# Batch 282: One missing Attica defendant with documented two-month pretrial custody.
# After merging, pull main and apply earlier pending batches in order.
# sudo -u www-data bash database/data/run-batch-282.sh --dry-run
# sudo -u www-data bash database/data/run-batch-282.sh
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-282.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch282.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 282 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
$checkCase = function ($case) {
    if (array_diff(array_keys($case), ["charges", "sentence", "convicted", "institution_id", "indicted", "imprisoned_for_months"])) { throw new \RuntimeException("Unexpected case field."); }
    Validator::make($case, ["charges" => "required|string|max:255", "sentence" => "required|string", "convicted" => "sometimes|string|max:255", "institution_id" => "sometimes|string", "indicted" => "sometimes|string|max:255", "imprisoned_for_months" => "sometimes|integer|in:2"])->validate();
};
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["anthony-williams-attica" => ["name" => "Anthony Williams", "affiliation" => ["Attica Brothers"], "state" => "New York", "era" => "1970s", "lat" => 42.8864, "lng" => -78.8784, "released" => false]];
    if (! isset($expected[$entry["key"]])) { throw new \RuntimeException("Unexpected reviewed identity."); }
    foreach ($expected[$entry["key"]] as $field => $value) { if (($entry["prisoner"][$field] ?? null) !== $value) { throw new \RuntimeException("Unexpected affiliation, location or custody status."); } }
    $reviewedCases = ["anthony-williams-attica" => [["charges" => "Kidnapping, unlawful imprisonment and coercion in the Attica uprising prosecution", "indicted" => "December 1972, according to the June 1975 Attica News notice", "convicted" => "Charges dropped; dismissal announced February 26, 1973", "sentence" => "Spent two months in Erie County Jail because of the Attica indictment, according to the March 1973 defense newsletter. Prosecutor Brian Malone announced the charges had been dropped on February 26, citing a possible identification error. This was pretrial detention, not a prison sentence. Exact jail admission and departure dates are unverified; the dismissal is not recorded as a physical release.", "imprisoned_for_months" => 2]]];
    if ($entry["case_dates"] !== [[]] || $entry["dates"] !== [] || $entry["prisoner"]["cases"] !== $reviewedCases[$entry["key"]]) { throw new \RuntimeException("Unreviewed case or date."); }
...
echo "B282-OK\n";
'
run "add-affiliation-prisoners" "B282-OK" "$ADD_CODE" || exit 1
echo "Batch 282 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
