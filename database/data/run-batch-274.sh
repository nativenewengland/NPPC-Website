#!/usr/bin/env bash
# Batch 274: 1 missing Justice 8 activist from the affiliation audit.
...
if [[ $# -eq 1 && "$1" == "--dry-run" ]]; then
    ...
    echo "Usage: bash database/data/run-batch-274.sh [--dry-run]" >&2
    exit 2
fi
...
$payload = json_decode(File::get(base_path("database/data/fixes/batch274.json")), true, 512, JSON_THROW_ON_ERROR);
if (($payload["batch"] ?? null) !== 274 || ($payload["expected_count"] ?? null) !== 1 || count($payload["entries"] ?? []) !== 1) {
...
foreach ($payload["entries"] as $entry) {
    ...
    $expected = ["gullit-eder-acevedo" => ["name" => "Gullit Eder Acevedo", "affiliation" => ["#Justice8"], "state" => "California", "era" => "2020s", "lat" => 34.54, "lng" => -117.29]];
    ...
    $approvedDates = ["gullit-eder-acevedo" => ["arrest_date" => ["year" => 2023, "month" => 12, "day" => 14]]];
...
echo "B274-OK\n";
'
run "add-affiliation-prisoners" "B274-OK" "$ADD_CODE" || exit 1
echo "Batch 274 complete (dry run: ${NPPC_BATCH_DRY_RUN})."
